...
TQE=function(objId,userConfig)
{
	...
	$.init=function(){
		...
		//code +=$._toobarCode($._config.toolbar, $._config.toolbarRight)
		code += '</div>'+
			//编辑区
	...
	$._popFormUI=function(id, caption, htm, cbOK, intInputNames, uploadUrl, uploadFileTypes, uploadFileSize, uploadType){
		...
		if(uploadUrl){
			...
			if(uploadUrl.substr(0,1)=='#'){//自定上传界面
				...
				url=TQE.url+'upload.htm?_url='+encodeURIComponent(uploadUrl)+'&field_name='+$.config.uploadFieldName+'&exts='+encodeURIComponent(uploadFileTypes)+'&max_size='+encodeURIComponent(uploadFileSize)+'&debug='+$.config.debug+'&upload_type='+uploadType;
			}
	...
	//};
	$._hook=function(){
		//if('visual'!=$.currentMode()) return; //!isIE || !$._config.enter2br ||
		var doc=$._getDoc();
		...
		//if($._config.autoClean) doc.body.onpaste=function(){setTimeout($.cleanWord,200);};
...
TQE.config={
	...
	//模板
	//插入媒体***时的模板代码,
	//flash	宏 {$_url} {$width} {$height}
	'tplFlash':'<embed  src="{$_url}" width="{$width}" height="{$height}" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" wmode="transparent" allowscriptaccess="always"></embed>',
	//flv视频 宏 {$_url} {$width} {$height} {$auto_start}
	'tplFlv':'<embed src="'+TQE.url+'flvPlayer.swf" flashvars="vcastr_file={$_url}&IsAutoPlay={$auto_start}" width="{$width}" height="{$height}" quality="high" bgcolor="#0E0E0E" name="play" align="middle" allowscriptaccess="sameDomain" allowfullscreen="true" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" wmode="opaque" />',
	//rm视频 宏 {$_url} {$width} {$height} {$auto_start}
	'tplRm':'<embed  src="{$_url}" width={$width} height={$height} autostart="{$auto_start}"type="audio/x-pn-realaudio-plugin" console="Clip1" controls="ImageWindow" ></embed>',
	//windows的媒体文件 宏 {$_url} {$width} {$height} {$auto_start}
	'tplVideo':'<object classid="clsid:6BF52A52-394A-11d3-B153-00C04F79FAA6"  type="application/x-ms-wmp" width="{$width}" height="{$height}"><PARAM NAME="URL" VALUE="{$_url}" /><PARAM NAME="autoStart" VALUE="{$auto_start}" /><PARAM NAME="invokeURLs" VALUE="false"><PARAM NAME="playCount" VALUE="100"><PARAM NAME="Volume" VALUE="100"><PARAM NAME="defaultFrame" VALUE="datawindow"></object>',
	//音乐播放 宏 {$_url} {$auto_start}
	'tplMusic':'<object id="tqeMP"  classid="clsid:6BF52A52-394A-11d3-B153-00C04F79FAA6"  type="application/x-ms-wmp" width="230" height="64"><PARAM NAME="URL" VALUE="{$_url}" /><PARAM NAME="autoStart" VALUE="{$auto_start}" /><PARAM NAME="invokeURLs" VALUE="false"><PARAM NAME="playCount" VALUE="100"><PARAM NAME="Volume" VALUE="100"><PARAM NAME="defaultFrame" VALUE="datawindow"></object>',
	//分页符代码
...
TQE.toolbarBottons={
	...
	createlink:{
		...
		click:function($,sender){
			...
			if(null==o){
				//var s='网址:<input type="text" id="_url" value="" size=45 /><br />打开:<select id="target"><option value="" >默认</option><option value="_blank">新窗口</option><option value="_self">当前窗口</option><option value="_parent">父窗口</option><option value="_top">顶层窗口</option></select>';
	...
	insertimage:{
		...
		click:function($,sender){
			...
			if(null==o){
				//var s='图片网址:<input type="text" id="_url" value="" size=40 /><br />替换文本:<input name="alt" type="text" id="alt" size="20" /><br />尺寸:<input name="width" type="text" id="width" size="4" /> &times; <input name="height" type="text" id="height" size="4" /> px &nbsp;<br />对齐:<select name="align" id="align"><option value="" >默认</option><option value="top">居上</option><option value="textTop">文本上方</option><option value="middle">居中</option><option value="absMiddle">绝对居中</option><option value="baseline">基线</option><option value="bottom">底部</option><option value="absBottom">绝对底部</option><option value="left">左</option><option value="right">右</option></select> &nbsp; &nbsp; 边框:<input name="border" type="text" id="border" size="5" />';
				var okClick=function(){
					TQE.hidePop();
					var es=this.form.elements;
	...
	insertflash:{
		...
		click:function($,sender){
			...
			if(null==o){
				//var s='Flash网址:<input type="text" id="_url" value="" size=40 /><br />尺寸:<input name="width" type="text" id="width" size="4" value="200" />&times;<input name="height" type="text" id="height" size="4" value="200" /> px';
	...
	insertmusic:{
		...
		click:function($,sender){
			...
			if(null==o){
				//var s='音频网址:<input type="text" id="_url" value="" size=40 /><br />自动播放:<select id="auto_start" name="auto_start"><option value="0">手动开始</option><option value="1">自动开始</option></select>';
				var okClick= function(){
					TQE.hidePop();
					var es=this.form.elements;
	...
	insertvideo:{
		...
		click:function($,sender){
			...
			if(null==o){
				//var s='视频网址:<input type="text" id="_url" value="" size=40 /><br />尺寸:<input name="width" type="text" id="width" size="4"  value="320"/>&times;<input name="height" type="text" id="height" size="4" value="240" /> px<br />自动播放:<select id="auto_start" name="auto_start"><option value="0">手动开始</option><option value="1">自动开始</option></select><br />视频类型:<select id="video_type" name="video_type"><option value="auto">自动识别</option><option value="flv">FLV</option><option value="rm">RMVB</option><option value="wm">其他视频</option></select>';
				var okClick= function(){
					TQE.hidePop();
					var es=this.form.elements;
		...
		click:function($,sender){
			...
			if(null==o){
				...
				var okClick= function(){
					...
					var es=this.form.elements;
					$.typeset();
					//$.insertVideo(es['_url'].value,es['auto_start'].value,es['width'].value,es['height'].value,es['video_type'].value);
					return false;
				};
	...
	splitpage:{
		...
		click:function($,sender){ 
			TQE.hidePop(); 
			//$.insertHtml($._config.tplSplitPage,'after');
